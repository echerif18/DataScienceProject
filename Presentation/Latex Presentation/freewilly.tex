...
\author{Eya CHERIF\\ N.Sabri OZTURK\\Jamil SYED RAWSHON\\
Seda TOPLAR\\ Özer YENIHAYAT}
\title{ \textbf{Graph Embedding with Self-Clustering}}
...
\begin{frame}{GEMSEC}
 ...
 use textblock to locate images in the frame

\begin{textblock*}{\paperwidth}(-0.14 \paperwidth, 10 mm)%
\hfill \includegraphics [width=0.44 \linewidth ]{pics/data}%
...
\begin{frame}{\underline{Com}munity \underline{E}mbedding (ComE)}
	\begin{itemize}

		\item Improves node embedding, community detection and community embedding jointly 		

		\begin{itemize}
			\item Based on their circular relation
			\item Community embedding improves node embedding by preserving high order proximity
		\end{itemize}
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=.3\linewidth]{"pics/circular relation"}
 	\caption[Circular relation between node embedding, community detection, community embedding]{Circular relation between node embedding, community detection, community embedding}
 	\label{fig:circular-relation}
 \end{figure}
...
\begin{frame}{Objective function}
	\begin{itemize}
		\item First order proximity
		\begin{itemize}
			\item Two neighbor has similar embedding		
			\item Objective function $O_{1}=-\sum_{\left(v_{i}, v_{j}\right) \in E} \log \sigma\left(\phi_{j}^{T} \phi_{i}\right)$	
		\end{itemize}
		\pause
		\item Second order proximity
		\begin{itemize}
			\item Both nodes share many context
			\item We adopt negative sampling
			\item Objective function $O_{2}=-\alpha \sum_{v_{i} \in V} \sum_{v_{j} \in C_{i}} \Delta_{i j}$ , where $\Delta_{i j}$ define how well $v_{i}$  generate it’s context
		\end{itemize}
		\pause
		\item Community detection and embedding
			\begin{itemize}
				\item Single objective function
				\item Objective function $O_{3}=-\frac{\beta}{K} \sum_{i=1}^{|V|} \log \sum_{k=1}^{K} \pi_{i k} \mathcal{N}\left(\phi_{i} | \psi_{k}, \Sigma_{k}\right)$
			\end{itemize}
		\pause
		\item Final objective function
			\begin{itemize}
				\item $\mathrm{O}=\mathrm{O}_{1}+\mathrm{O}_{2}+\mathrm{O}_{3}$
			\end{itemize}
		\pause
	\end{itemize}
...
\item Community detection is useful for ...
...
systems, pp. 539-547, (2012).
	...
	\bibitem{ComE}Cavallari, Sandro and Zheng, Vincent W and Cai, Hongyun and Chang, Kevin Chen-Chuan and Cambria, Erik, "Learning community embedding with community detection and node embedding on graphs"
